...
export function buildProposal({ repository, headSha, group, references, history = [] }) {
    const { owner, repo } = normalizeRepository(repository);
    ...
    const labels = ['cookie', 'agentic-workflows'];
    if (group.kind === 'ignore' || group.path.startsWith('test/')) {
        labels.push('Test Debt');
    }
    ...
    const body = [
        ...
        '',
        `Target branch: **main**. Follow-up kind: **${group.kind}**.`,
        '',
        ...
        '',
        ...(group.kind === 'ignore' ? [
            '1. Work against `main`; check the source, all prerequisites above, dependency consumption, and required platform.',
            '2. Remove the relevant Ignore for the listed tests, preserving unrelated ignores and conditions.',
            `3. Use the repository [run-tests skill](${base}/blob/main/.github/skills/run-tests/SKILL.md)`,
            ...
            '1. Work against `main`; first verify the stated prerequisites, fixed-version consumption, and behavior',
            ...
            `   [run-tests skill](${base}/blob/main/.github/skills/run-tests/SKILL.md) and verify actual execution.`,
...
export async function finalize({ github, repository, headSha, actions, dryRun, logger = console }) {
    ...
    async function readTracking() {
        // Both complete listings must succeed before any proposed mutation.
    ...
    for (const { group, references } of eligible) {
        ...
        if ((dryRun ? result.proposed.length : result.created.length) >= maximumNewIssues) {
            result.skipped.push({ candidateIds: group.candidateIds, reason: 'run-limit' });
            continue;
        }
        if (!dryRun) {
            try {
        ...
        const proposal = buildProposal({ repository, headSha, group, references, history });
