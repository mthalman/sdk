...
test('fixed ignore template and code-owned labels include pinned evidence and actual-test follow-up', async () => {
    ...
    assert.deepEqual(payload.labels, ['cookie', 'agentic-workflows', 'Test Debt']);
...
test('TODO/workaround tasks check prerequisites/backcompat and only test sites receive Test Debt', async () => {
    for (const kind of ['todo', 'workaround']) {
        ...
        assert.deepEqual(proposal.labels, ['cookie', 'agentic-workflows']);
        ...
        assert.deepEqual(testProposal.labels, ['cookie', 'agentic-workflows', 'Test Debt']);
    }
});
...
test('three-issue cap is hardcoded and excludes duplicates across cached/fresh/repeated actions', async () => {
    const actions = Array.from({ length: 7 }, (_, i) => action({
        candidateId: `candidate-${i}`, testNames: [`Tests.FeatureTests.Case${i}`],
    }));
    const api = mock();
    const result = await run(api, [...actions, ...actions], { maximumNewIssues: 100 });
    assert.equal(result.created.length, 3);
    assert.equal(api.calls.writes.length, 3);
    assert.equal(result.skipped.filter(skip => skip.reason === 'run-limit').length, 4);
    assert.equal(api.calls.reads.length, 1);
    const dry = await run(mock(), [...actions, ...actions], { dryRun: true });
    assert.equal(dry.proposed.length, 3);
    assert.equal(dry.skipped.filter(skip => skip.reason === 'run-limit').length, 4);
});
...
test('reconciled creates count against the same hard three-issue cap', async () => {
    const api = mock({ create: async (args, state, calls) => {
        state.open.push({ number: 100 + calls.writes.length, state: 'open', body: args.body });
        throw { status: 502 };
    } });
    const inputs = Array.from({ length: 5 }, (_, index) => action({
        candidateId: `candidate-${index}`, testNames: [`Tests.FeatureTests.Case${index}`],
    }));
    const result = await run(api, inputs);
    assert.equal(api.calls.writes.length, 3);
    ...
    assert.equal(result.skipped.filter(skip => skip.reason === 'run-limit').length, 2);
});
