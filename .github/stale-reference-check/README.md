...
This is a **fork-only deployment**. The [driver](../workflows/stale-reference-check.yml)
runs only by manual dispatch on `mthalman/sdk` `main`; there is no schedule.
It creates at most **three tracking issues total across all attempts**, not three
per attempt. It never changes source,
opens a pull request, comments on an existing issue, or removes an Ignore.

[`fork-only.mjs`](fork-only.mjs) wraps the injected Octokit client without changing
the shared discovery, interpretation, or finalization helpers. It refuses any
other repository or branch and exposes only reference reads, issue listings, and
guarded issue creation. Before every creation, it fully paginates **all** fork
issues, open and closed, without label filtering. Marked stale-reference issues
and all issues authored by `github-actions[bot]` consume the cumulative budget;
the latter intentionally overcounts unrelated Actions issues and survives edited
bodies and labels. Pull requests do not count. Do not delete trial issues:
GitHub's issue listing cannot account for deleted history.

Created issues have a `[Fork workflow test]` title prefix and an explicit test-output
notice above the unchanged body markers. They are not upstream work requests.

Malformed, incomplete, or failed history reads fail closed. Creation is serialized
and never retried; an ambiguous creation result or safety refusal blocks further
writes and fails the job, even if the shared finalizer reconciles an issue.
Run-level concurrency serializes attempts. Preview never creates issues or saves
the production interpretation cache, including after the lifetime budget is spent.
Upstream blocker URLs remain read-only.
...
## Filed tasks and disabled assignment
...
that label. Issue Monster and all unrelated fork workflows must remain disabled
throughout this trial. This deployment never dispatches Issue Monster or assigns
work automatically.

For ignored tests, the task starts by removing the relevant Ignore and following
the upstream [`run-tests` skill](https://github.com/dotnet/sdk/blob/main/.github/skills/run-tests/SKILL.md) for the smallest appropriate
...
- `refresh_cache: true` ignores cached interpretations for collection. Batch
  limits still apply.
- Live manual runs retain validated interpretations and share the lifetime cap.
...
these tests only by manual dispatch on `mthalman/sdk` `main`, without invoking
the interpreter or granting issue-write permissions.
...
```powershell
gh aw compile stale-reference-interpret --no-check-update --validate --action-mode action --action-tag v0.88.7
```
